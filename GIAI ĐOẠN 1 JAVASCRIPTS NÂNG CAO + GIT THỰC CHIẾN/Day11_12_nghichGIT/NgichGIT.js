...
const loc = tuKhoa.filter((x, index) => {
  return tuKhoa.indexOf(x) === index; // So sanh trong mảng tukhoa xem có x đang duyệt ở vị trí thứ mấy , duyệt lần đầu 
});
console.log(loc);
...
arr.sort((x, y) => x - y); // x-y < 0 thì sẽ xếp x trước y nếu không dùng hàm so sánh thì sẽ tự dộng nhận các giá trị theo string , tôi không nhớ thứ tự string của các số
console.log(arr);
